...
import net.minecraft.util.Screenshot.ScreenshotTaker;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
			...
			while (this.running) {
			   if (!this.hasCrashed || this.crashReporter == null) {
				  try {
					 ...
					 if (!Settings.getBool(MLibSettings.FIX_MEMORY_LEAKS_MCGC)) {
						System.gc();
					 }
				  }
			   }
		 ...
		 public String formatString(String str) {
			try {
			   return String.format(str, new Object[] {
					 GameSettings.getKeyDisplayString(Minecraft.this.gameSettings.keyBindInventory.getKeyCode()) });
			}
		 ...
		 for (DisplayMode displaymode1 : macDisplayModes) {
			...
			if (!flag) {
			   ...
			   while (true) {
				  ...
				  if (displaymode3.getBitsPerPixel() == 32 && displaymode3.getWidth() == displaymode1.getWidth() / 2
   ...
   public void draw(int posX, int posY, int texU, int texV, int width, int height, int red, int green, int blue,
	  ...
	  worldrenderer.pos((double) posX, (double) (posY + height), 0.0D)
			.tex((double) ((float) texU * f), (double) ((float) (texV + height) * f1)).color(red, green, blue, alpha)
			.endVertex();
	  ...
	  worldrenderer.pos((double) posX, (double) posY, 0.0D)
			.tex((double) ((float) texU * f), (double) ((float) texV * f1)).color(red, green, blue, alpha).endVertex();
   ...
   private void runGameLoop() throws IOException {
	  ...
	  debugFPS = FPSCalculator.INSTANCE.getCurrentFPS();
	  while (getSystemTime() >= this.debugUpdateTime + 500L) {
   ...
   public int getLimitFramerate() {
	  return this.theWorld == null && this.currentScreen != null
			? (Settings.getBool(MLibSettings.REMOVEFPSLIMIT) ? Integer.MAX_VALUE : 30)
	  ...
	  if (this.mcProfiler.profilingEnabled) {
		 ...
		 for (int l = 0; l < list.size(); ++l) {
			...
			for (int j2 = i1; j2 >= 0; --j2) {
			   float f = (float) ((d0 + profiler$result1.field_76332_a * (double) j2 / (double) i1) * Math.PI * 2.0D
					 / 100.0D);
			...
			for (int i3 = i1; i3 >= 0; --i3) {
			   float f3 = (float) ((d0 + profiler$result1.field_76332_a * (double) i3 / (double) i1) * Math.PI * 2.0D
					 / 100.0D);
		 ...
		 for (int k2 = 0; k2 < list.size(); ++k2) {
			...
			this.fontRendererObj.drawStringWithShadow(s1 = decimalformat.format(profiler$result2.field_76332_a) + "%",
				  (float) (j + i - 50 - this.fontRendererObj.getStringWidth(s1)), (float) (k + i / 2 + k2 * 8 + 20),
				  profiler$result2.getColor());
			this.fontRendererObj.drawStringWithShadow(s1 = decimalformat.format(profiler$result2.field_76330_b) + "%",
   ...
   public void runTick() throws IOException {
	  ...
	  if (Settings.getBool(MLibSettings.REMOVELEFTCLICKCOUNTERMC)) {
		 this.leftClickCounter = 0;
	  }
			   ...
			   if (j != 0) {
				  if (this.thePlayer.isSpectator()) {
					 ...
					 if (this.ingameGUI.getSpectatorGui().func_175262_a()) {
						...
						float f = MathHelper.clamp_float(this.thePlayer.capabilities.getFlySpeed() + (float) j * 0.005F,
							  0.0F, 0.2F);
    ...
    */
   public void loadWorld(WorldClient worldClientIn) { this.loadWorld(worldClientIn, ""); }
   ...
   public CrashReport addGraphicsAndWorldToCrashReport(CrashReport theCrash) {
	  theCrash.getCategory().addCrashSectionCallable("Launched Version", new Callable<String>() {
		 public String call() throws Exception { return Minecraft.this.launchedVersion; }
	  });
	  theCrash.getCategory().addCrashSectionCallable("LWJGL", new Callable<String>() {
		 public String call() { return Sys.getVersion(); }
	  });
	  ...
	  theCrash.getCategory().addCrashSectionCallable("GL Caps", new Callable<String>() {
		 public String call() { return OpenGlHelper.getLogText(); }
	  });
	  theCrash.getCategory().addCrashSectionCallable("Using VBOs", new Callable<String>() {
		 public String call() { return Minecraft.this.gameSettings.useVbo ? "Yes" : "No"; }
	  });
	  ...
	  theCrash.getCategory().addCrashSectionCallable("Type", new Callable<String>() {
		 public String call() throws Exception { return "Client (map_client.txt)"; }
	  });
	  ...
	  theCrash.getCategory().addCrashSectionCallable("CPU", new Callable<String>() {
		 public String call() { return OpenGlHelper.getCpu(); }
	  });
	  if (this.theWorld != null) {
   ...
   public ListenableFuture<Object> scheduleResourcesRefresh() {
	  return this.addScheduledTask(new Runnable() { public void run() { Minecraft.this.refreshResources(); } });
   }

   public void addServerStatsToSnooper(PlayerUsageSnooper playerSnooper) {
	  playerSnooper.addClientStat("fps", Integer.valueOf(FPSCalculator.INSTANCE.getCurrentFPS()));
    ...
    */
   public void setServerData(ServerData serverDataIn) { this.currentServerData = serverDataIn; }
   ...
   public static int getDebugFPS() { return FPSCalculator.INSTANCE.getCurrentFPS(); }
    ...
    */
   public void setConnectedToRealms(boolean isConnected) { this.connectedToRealms = isConnected; }
}
