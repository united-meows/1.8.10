...
public class ItemRenderer {
		 ...
		 if (this.itemToRender != null) {
			if (this.itemToRender.getItem() instanceof ItemMap) {
			   ...
			   case DRINK:
				  ...
				  break;
			   case BLOCK:
				  float oldAnimationProgress = MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI);
				  if (oldAnimations) {
	  ...
	  if (this.mc.thePlayer.isEntityInsideOpaqueBlock()) {
		 ...
		 if (iblockstate.getBlock().getRenderType() != -1) {
			...
			if (!Reflector.callBoolean(Reflector.ForgeEventFactory_renderBlockOverlay,
				  new Object[] { this.mc.thePlayer, Float.valueOf(partialTicks), object, iblockstate, blockpos })) {
   ...
   /**
    * Renders a texture that warps around based on the direction the player is looking. Texture needs to be bound before being called. Used for the water overlay. Args:
    * parialTickTime
    * 
    * @param partialTicks Partial ticks
    */
		 ...
		 if (!this.itemToRender.getIsItemStackEqual(itemstack)) {
			if (Reflector.ForgeItem_shouldCauseReequipAnimation.exists()) {
			   boolean flag1 = Reflector.callBoolean(this.itemToRender.getItem(),
					 Reflector.ForgeItem_shouldCauseReequipAnimation, new Object[] { this.itemToRender, itemstack,
