...
public class EnchantmentHelper {
   ...
   private static void applyEnchantmentModifierArray(EnchantmentHelper.IModifier modifier, ItemStack[] stacks) {
	  for (ItemStack itemstack : stacks) { applyEnchantmentModifier(modifier, itemstack); }
   }
   ...
   public static int getEnchantmentModifierDamage(ItemStack[] stacks, DamageSource source) {
	  /**
	   * Fix a memory leak caused by the world object still being pointed to by the static ENCHANTMENT_ fields. Instead of modifying those static fields, make a new instance of
	   * IModifier every use. Overwriting these methods is necessary, it would require a redirect for every single line otherwise.
	   * <p>
   ...
   public static void applyThornEnchantments(EntityLivingBase p_151384_0_, Entity p_151384_1_) {
	  if (Settings.getBool(MLibSettings.FIX_MEMORY_LEAKS_ENCHANTMENTHELPERMCC)) {
		 ...
		 if (p_151384_0_ != null) {
			applyEnchantmentModifierArray(enchantmentIteratorHurt, p_151384_0_.getInventory());
		 }
		 if (p_151384_1_ instanceof EntityPlayer) {
			applyEnchantmentModifier(enchantmentIteratorHurt, p_151384_0_.getHeldItem());
		 }
	  }
	  else {
   ...
   public static void applyArthropodEnchantments(EntityLivingBase p_151385_0_, Entity p_151385_1_) {
	    if (Settings.getBool(MLibSettings.FIX_MEMORY_LEAKS_ENCHANTMENTHELPERMCC)) {
		 ...
		 if (p_151385_0_ != null) {
			applyEnchantmentModifierArray(enchantmentIteratorDamage, p_151385_0_.getInventory());
		 }
		 if (p_151385_0_ instanceof EntityPlayer) {
			applyEnchantmentModifier(enchantmentIteratorDamage, p_151385_0_.getHeldItem());
		 }
	  }
	  else {
   ...
   /**
    * Returns the enchantability of itemstack, using a separate calculation for each enchantNum (0, 1 or 2), cutting to the max enchantability power of the table, which is locked
    * to a max of 15.
    */
