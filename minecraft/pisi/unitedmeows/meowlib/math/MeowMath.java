...
public class MeowMath {
   ...
   public static float sinRaven(float rad) { return BF_sinFull[(int) (rad * BF_radToIndex) & BF_SIN_MASK]; }

   public static float cosRaven(float rad) { return sinRaven(rad + BF_SIN_TO_COS); }

   private static final int[] SINE_TABLE_INT = new int[16384 + 1];
   private static final float SINE_TABLE_MIDPOINT;
   static {
	  for (int i = 0; i < 65536; ++i) { SIN_TABLE[i] = (float) Math.sin((double) i * Math.PI * 2.0D / 65536.0D); }
	  for (int i = 0; i < SINE_TABLE_INT.length; i++) { SINE_TABLE_INT[i] = Float.floatToRawIntBits(SIN_TABLE[i]); }
   ...
   // ok im not sure if this is faster or not but this is more accurate -ghost
   public static float sin(float f) { return lookup((int) (f * 10430.38) & 0xFFFF); }

   public static float sin(double d) { return lookup((int) (d * 10430.38) & 0xFFFF); }

   public static float cos(float f) { return lookup((int) (f * 10430.38 + 16384.0) & 0xFFFF); }

   public static float cos(double f) { return lookup((int) (f * 10430.38 + 16384.0) & 0xFFFF); }
   ...
   private static float lookup(int index) {
	  ...
	  // thank you some random guy on stackoverflow
	  pos &= 0x7fff;
